				...
				<div id="tb">
					<div id="filter_tgl" class="input-group" style="display: inline;">
						<button class="btn btn-default" id="daterange-btn">
							<i class="fa fa-calendar"></i> <div id="reportrange" style="display: inline;"><span></span></div>
						...
						<input id="fr_anggota_id" name="fr_anggota_id" value="" style="width:100px; height:34px;">
						<input type="text" id="fr_bulan" name="fr_bulan" value="" style="display: inline-block; width: 50px; line-height: 20px; vertical-align: middle; background-color:#ddd;" readonly="true" title="Filter Untuk Per Tanggal 18 Sampai 17" />
						<a href="javascript:void(0);" id="fm_filter" class="btn btn-primary"><i class="fa fa-filter"></i> FILTER</a>
						<a href="javascript:void(0);" id="fm_cetak" class="btn btn-info" onclick="cetak_laporan();"><i class="fa fa-print"></i> CETAK</a>
						<a href="javascript:void(0);" id="fm_exp_xlx" class="btn btn-success" onclick="exp_xlx();"><i class="fa fa-table"></i> Export Excel</a>
					</div>
				...
				<table 
					...
					<thead>
						<tr>
							...
							<th data-field="status" data-sortable="true" data-align="center" data-halign="center" data-valign="middle" data-formatter="status_ft">Jenis Pinjaman</th>							
							<?php /*
							<th data-field="jenis" data-sortable="true" data-valign="middle" data-align="center" data-halign="center">Jenis</th>
							*/ ?>
							<th data-field="nominal" data-sortable="true" data-valign="middle" data-align="right" data-halign="center" data-formatter="nominal_ft">Nominal Pinjaman</th>
							<?php /*
...
<script type="text/javascript">
	...
	function nominal_ft(value, row, index) {
		var nsi_out = '';
		nsi_out += '<span class="text-muted">Pinjaman: Rp </span> '+row.nominal;
		nsi_out += '<br><span class="text-muted">Lama Angsuran:</span> '+row.lama_ags+' <span class="text-muted">bln</span>';
	...
	function sisa_ft(value, row, index) {
		...
		nsi_out += '<br><span class="text-muted">Sisa Ags Per Bln:</span> Rp '+row.sisa_ags_rp;
		nsi_out += '<br><span class="text-red">Max Ags Per Bln:</span> Rp '+row.pengajuan_max;
		return nsi_out;
	}
	...
	function aksi_ft(value, row, index) {
		...
		<?php if($this->session->userdata('level') != 'operator') { ?>
			var link_diterima = '<li><a data-data_aksi="Setuju" data-data_id="'+row.id+'" class="a_diterima" href="javascript:void(0);"><i class="fa fa-check-circle"></i> Setujui</a></li>';
			var link_ditolak = '<li><a data-data_aksi="Tolak" data-data_id="'+row.id+'" class="a_ditolak" href="javascript:void(0);"><i class="fa fa-times-circle"></i> Tolak</a></li>';
			...
			var link_batal = '<li><a data-data_aksi="Batal" data-data_id="'+row.id+'" class="a_dibatal" href="javascript:void(0);"><i class="fa fa-trash-o"></i> Batal</a></li>';
		...
		<?php if($this->session->userdata('level') != 'pinjaman') { ?>
			var link_terlaksana = ' <li><a data-data_aksi="Terlaksana" data-data_id="'+row.id+'" class="a_dilaksanakan" href="javascript:void(0);"><i class="fa fa-rocket"></i> Sudah Dilaksanakan</a></li>';
			var link_belum = ' <li><a data-data_aksi="Belum" data-data_id="'+row.id+'" class="a_belum" href="javascript:void(0);"><i class="fa fa-rocket"></i> Belum Dilaksanakan</a></li>';
			if(row.status == 1) {
				out += link_terlaksana;
			}
			if(row.status == 3) {
		...
		<?php if($this->session->userdata('level') == 'admin') { ?>
			...
			out += ' <li><a data-data_aksi="Hapus" data-data_id="'+row.id+'" class="a_hapus" href="javascript:void(0);"><i class="fa fa-times-circle"></i> Hapus</a></li>';
		<?php } ?>
		out += '		<li role="separator" class="divider"></li>';
