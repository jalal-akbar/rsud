...
<head>
	<meta charset="UTF-8">
	<title>Member - KPRI HUSADA</title>
			...
			<?php
				...
				if($data_pengajuan) {
					...
					if($data_pengajuan->status == 3) {
						$status = '<span class="text-success"><i class="fa fa-money"></i> Uang Sudah Diterima</span>';
					}
					if($data_pengajuan->status == 4) {
						$status = '<span class="text-success"><i class="fa fa-rocket"></i>  BATAL</span>';
					}
					$isi .= '<div class="alert alert-info">Pengajuan Pinjaman : <strong>'.jin_date_ina($data_pengajuan->tgl_update, 'full', true).'</strong> Sebesar Rp <strong>'.number_format($data_pengajuan->nominal).'</strong> Status : <strong>'.$status.'</strong> <strong>(Lebih Lengkap Lihat di Menu DATA PENGAJUAN)</strong></div>';
				} else {
					...
					<tr>
						...
						<td> 
							<table class="table table-responsive">
								<tr><td> NIK / ID Anggota : '.$row->identitas.' - '.'AG'.sprintf('%04d', $user_id).' </td></tr>
								<tr><td> Nama : <b>'.strtoupper($row->nama).'</b></td></tr>
								<tr><td> Jenis Kelamin : '.$jk.' </td></tr>
								<tr><td> Departemen : '.$row->departement.'-'.$row->pekerjaan.'</td></tr>
								<tr><td> Alamat  : '.$row->alamat.' HP : '.$row->notelp.' </td></tr>
							</table>
						</td>
						</tr>
						</table>
