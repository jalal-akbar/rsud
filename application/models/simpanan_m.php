...
class Simpanan_m extends CI_Model {
	...
	function get_data_transaksi_ajax($offset, $limit, $q='', $sort, $order) {
		$sql = "SELECT tbl_trans_sp.* FROM tbl_trans_sp 
				LEFT JOIN tbl_anggota ON tbl_anggota.id = tbl_trans_sp.anggota_id
		...
		if(is_array($q)) {
			if($q['kode_transaksi'] != '') {
				...
				//$q['kode_transaksi'] = str_replace('AG', '', $q['kode_transaksi']);
				//$q['kode_transaksi'] = $q['kode_transaksi'] * 1;
							//tbl_trans_sp.id LIKE '".$q['kode_transaksi']."' OR 
							//tbl_trans_sp.anggota_id LIKE '".$q['kode_transaksi']."' OR
				$sql .=" AND (
							tbl_anggota.nama LIKE '%".$q['kode_transaksi']."%' 
						) ";
			} else {
				if($q['cari_simpanan'] != '') {
