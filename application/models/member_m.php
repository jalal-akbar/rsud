...
class Member_m extends CI_Model {
	...
	function pengajuan_simpan() {
		...
		$rek_lain = '';
		if($jenis_bayar == 'Bank Transfer' && $pil_rek == '2') {
			$rek_lain = trim(strip_tags($this->input->post('rek_lain')));
		}
		$nominal = preg_replace('/\D/', '', $this->input->post('nominal'));
		if(date("d") >= 18) {
			$bln_1 = date("Y-m") . '-18';
			$bln_2 = date("Y-m", strtotime("+1 month")) . '-17';
		} else {
			$bln_1 = date("Y-m", strtotime("-1 month")) . '-18';
			$bln_2 = date("Y-m") . '-17';
		}
		...
		if(is_array($data)) {
			...
			foreach ($data as $rows) {
				...
				foreach ($rows as $key => $val) {
					...
					if($key == 'O') { $pair['nama_bank'] = $val; }
					if($key == 'P') { $pair['no_rek'] = $val; }
					if($key == 'Q') { $pair['gaji_net'] = $val; }
				}
				$pair['jabatan_id'] = 2;
				$pair_arr[] = $pair;
			}
	...
	function get_pengajuan() {
		...
		$order_by = " ORDER BY tbl_pengajuan.tgl_input DESC";
		$sql_limit = " LIMIT ".$offset.",".$limit." ";
		
		$sql_tampil = "	SELECT tbl_pengajuan.*,  tbl_anggota.nama_bank, tbl_anggota.no_rek
						FROM tbl_pengajuan 
						LEFT JOIN tbl_anggota ON tbl_anggota.id = tbl_pengajuan.anggota_id
						WHERE 1=1 ".$where." ".$order_by." ".$sql_limit;
		$query = $this->db->query($sql_tampil);
		$data_list = $query->result();

		$sql_total = "SELECT tbl_pengajuan.id FROM tbl_pengajuan WHERE 1=1 ".$where." ";
