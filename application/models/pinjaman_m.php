...
class Pinjaman_m extends CI_Model {
	...
	function get_pengajuan() {
		...
		$fr_anggota_id = isset($_POST['fr_anggota_id']) ? $_POST['fr_anggota_id'] : '';
		...
		if($fr_bulan != '') {
			...
			$bln_dari = substr($bln_dari, 0, 7) . '-18';
			$bln_samp = $fr_bulan . '-17';
		...
		if($fr_anggota_id != '') {
			//$filter_range = false;
			$where .= " AND a.anggota_id = '".$fr_anggota_id."' ";
		}
		...
		$sql_tampil = "SELECT 
			...
			b.identitas AS identitas, b.nama AS nama, b.departement AS departement, b.gaji_net AS gaji_net, a.jenis_bayar AS jenis_bayar, a.rek_lain AS rek_lain, b.no_rek AS no_rek
		...
		foreach ($data_list as $key => $val) {
			// nominal ags
			$angsuran_pokok = ($val->nominal / $val->lama_ags);
			...
			if($conf_bunga['pinjaman_bunga_tipe'] == 'A') {
				...
				$biaya_bunga = ($val->nominal * $persen_bunga) / 100;
			}
			$num_ags = $angsuran_pokok + $biaya_bunga + $biaya_admin;
			$val->num_ags = number_format($num_ags);
			...
			$val->sisa_ags_rp = number_format($sisa_p['sisa_ags_rp']);

			$gaji_net = (int) $gaji_net_arr[$val->gaji_net]['nominal'];
			$pengajuan_max = get_pengajuan_max($gaji_net);
			$val->pengajuan_max = number_format($pengajuan_max);

			$val->rek_lain = nl2br($val->rek_lain);
	...
	function get_pengajuan_cetak() {
		...
		$fr_anggota_id = isset($_REQUEST['fr_anggota_id']) ? $_REQUEST['fr_anggota_id'] : '';
		...
		$sql_tampil = "SELECT 
			a.id AS id, a.no_ajuan AS no_ajuan, a.ajuan_id AS ajuan_id, a.anggota_id AS anggota_id, a.tgl_input AS tgl_input, a.jenis AS jenis, a.nominal AS nominal, a.lama_ags AS lama_ags, a.keterangan AS keterangan, a.status AS status, a.alasan AS alasan, a.tgl_update AS tgl_update, a.tgl_cair AS tgl_cair, a.jenis_bayar AS jenis_bayar, a.rek_lain AS rek_lain,
			b.identitas AS identitas, b.nama AS nama, b.departement AS departement, b.no_rek AS no_rek
			FROM tbl_pengajuan AS a
			LEFT JOIN tbl_anggota AS b ON b.id = a.anggota_id
		...
		foreach ($data_list as $key => $val) {
			...
			// jenis bayar
			$val->jenis_bayar_txt = '';
			if($val->jenis_bayar == 'Bank Transfer') {
				if(empty($val->rek_lain)) {
					//$val->jenis_bayar_txt = 'Rek Pribadi';
					$val->jenis_bayar_txt = 'Danamon - '.$val->no_rek;
				} else {
					$split_rek = preg_split('/\r\n|\r|\n/', $val->rek_lain);
					$val->jenis_bayar_txt = implode(' - ', $split_rek);
				}
			} else {
				$val->jenis_bayar_txt = 'Tunai';
			}
	...
	function get_sisa_pinjaman($anggota_id) {
		...
		if($query->num_rows() > 0) {
			...
			foreach ($result as $row) {
				...
				$sisa_tagihan += $row->tagihan - ($row->bln_sudah_angsur * $row->ags_per_bulan);
				$sisa_ags += $row->lama_angsuran - $this->get_sisa_ags($row->id);
				$sisa_ags_rp += $row->ags_per_bulan;
			}
			$out['sisa_jml'] = $item;
			$out['sisa_tagihan'] = $sisa_tagihan;
			$out['sisa_ags'] = $sisa_ags;
			$out['sisa_ags_rp'] = $sisa_ags_rp;
			return $out;
		} else {
			return $out;
		}
	...
	function get_sisa_pinjaman_minus_satu_bln($anggota_id) {
		$this->db->select('*');
		$this->db->from('v_hitung_pinjaman');
		$this->db->where('lunas', 'Belum');
		$this->db->where('anggota_id', $anggota_id);
		...
		if($query->num_rows() > 0) {
			...
			foreach ($result as $row) {
				...
				//$sisa_tagihan += $row->tagihan - $this->get_jml_bayar($row->id);
				$sisa_tagihan += $row->tagihan - ($row->bln_sudah_angsur * $row->ags_per_bulan);
				if($row->lama_angsuran == 1) {
					$sisa_ags_rp += $row->ags_per_bulan;
					$sisa_ags += $row->lama_angsuran - $this->get_sisa_ags($row->id);
				} else {
					$sisa_lama_angsuran = $row->lama_angsuran - $row->bln_sudah_angsur;
					if($sisa_lama_angsuran > 1) {
						$sisa_ags_rp += $row->ags_per_bulan;
						$sisa_ags += $row->lama_angsuran - $this->get_sisa_ags($row->id);
					}
				}
	...
	//cek data pengajuan Pending berdasarkan ANGGOTA ID
	function cek_data_pengajuan_pending($anggota_id) {
		$this->db->select('id');
